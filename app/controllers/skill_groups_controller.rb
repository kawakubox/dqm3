class SkillGroupsController < ApplicationController
  ...
  # GET /skill_groups or /skill_groups.json
  def index
    @skill_groups = SkillGroup.all
  end

  # GET /skill_groups/1 or /skill_groups/1.json
  def show
  end
  ...
  # POST /skill_groups or /skill_groups.json
  def create
    @skill_group = SkillGroup.new(skill_group_params)

    respond_to do |format|
      if @skill_group.save
        format.html { redirect_to skill_group_url(@skill_group), notice: "Skill group was successfully created." }
  ...
  # PATCH/PUT /skill_groups/1 or /skill_groups/1.json
  def update
    respond_to do |format|
      if @skill_group.update(skill_group_params)
        format.html { redirect_to skill_group_url(@skill_group), notice: "Skill group was successfully updated." }
  ...
  # DELETE /skill_groups/1 or /skill_groups/1.json
  def destroy
    @skill_group.destroy!
  ...
  private
    ...
    def skill_group_params
      params.require(:skill_group).permit(:name)
    end
end
